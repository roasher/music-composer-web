define(["../fileplayer/Range"],
    function (Range) {
        var noteSelector = function (container, id, defaultRange, partNumber) {
        ...
        noteSelector.prototype.setSelectValues = function(range) {
            for (optionNumber = 0; optionNumber < this.voice.options.length; optionNumber++) {
                var option = this.voice.options[optionNumber];
                if (option.value === range.lowerNote.toString() || option.value === range.upperNote.toString()) {
